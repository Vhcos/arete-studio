...
function onNext() {
  ...
  return (
    <div>
      <h1 className="text-xl font-semibold mb-1">Paso FINAL · Confirmación</h1>
      ...
      <div className="mt-6 flex gap-3">
        <button onClick={() => router.push("/wizard/step-8")} className="rounded-lg border px-4 py-2">Atrás</button>
        {SHOW_IA && (
          <button onClick={onIA} className="rounded-lg bg-slate-900 text-white px-4 py-2">Generar plan (IA)</button>
        )}
        <button onClick={onNext} className="rounded-lg bg-green-600 text-white px-4 py-2">Ir al Tablero </button>
      </div>
      <UpsellBanner />
